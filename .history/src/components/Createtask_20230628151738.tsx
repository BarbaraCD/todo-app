...
export function Createtask() {
  ...
  async function fetchTasks() {
    ...
    const fetchedTasks = response.reverse() // Inverte a ordem das tarefas
    const tasksWithReversedIds = fetchedTasks.map((task, index) => ({
      ...task,
      id: index + 1, // Reverte a ordem dos IDs
    }))
    setTasks(tasksWithReversedIds)
  }
  ...
  const removeTask = async (id: number) => {
    await deleteTask(id)
    const updatedTasks = tasks.filter((task) => task.id !== id) // Filtra a tarefa removida
      ...
      {tasks.length > 0 &&
        tasks.map((task) => (
          <Tasks
            ...
            title={task.title}
            handleDelete={() => {
              removeTask(task.id) // Passa o ID correto para a remoção da tarefa
            }}
          />
        ))}
    </div>
